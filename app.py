...
# --- SAYFA AYARLARI ---
st.set_page_config(page_title="Momentum Rallisi Botu - Trend Avcısı", page_icon="🚀", layout="wide")
...
# --- 5. STRATEJİ MOTORU: MOMENTUM RALLİSİ ---
class QuantStrategy:
    ...
    def analiz_et(self, hisse_kodu):
        ...
        bb_upper = float(son_gun['BB_Upper'])
        ...
        # TREND ŞARTLARI: Ortalamalar düzgün dizilmiş, ADX (Trend Gücü) 20'nin üstünde olmalı.
        if fiyat > sma_50 and sma_50 > sma_200 and mac_deger > mac_signal and adx_deger > 20:
            if fiyat >= bb_upper * 0.98: 
                mom_skor += 40
                mom_nedenler.append("Bollinger Üst Bant Kırılımı 🚀")
            if hacim > hacim_ort * 1.3: 
                mom_skor += 30
                mom_nedenler.append("Hacim Patlaması")
            if rsi > 60 and rsi < 85: 
                mom_skor += 30
                mom_nedenler.append(f"Güçlü Trend (ADX: {int(adx_deger)})")

            if self.piyasa_rejimi == "BEAR": mom_skor -= 20; mom_nedenler.append("Ayı Piyasası İskontosu")

        # FİLTRE: Skor 70'i geçmeden sinyal üretme
        if mom_skor >= 70:
            ...
            return {
                "Hisse": temiz_kod, "Karar": "🚀 MOMENTUM RALLİSİ", "Skor": f"%{min(mom_skor, 100)}",
                "Tip": "Momentum", "Renk": "#004d40",
                "Kaynak": kaynak, "Fiyat": round(fiyat, 2), "Stop Loss": stop_loss, "Hedef Fiyat": hedef_fiyat,
                "Fırsat Özeti": " | ".join(mom_nedenler[:3]),
...
# --- 6. ARAYÜZ (UI) ---
def ui_olustur():
    st.title("🚀 Momentum Rallisi Botu (Trend Avcısı)")
    st.markdown("Sadece Hacim patlaması yapan, Bollinger üst bandını kıran ve ADX trend gücü onaylı tahtaları tarar.")
    st.markdown("---")
    ...
    if hizli_tarama_butonu or ana_tarama_butonu:
        ...
        if bulunan_firsatlar:
            st.success(f"🚨 Tarama Tamamlandı! Kriterlere uyan {len(bulunan_firsatlar)} trend fırsatı bulundu.")
...
<p style="font-size: 11px; text-align: right; color: #c8e6c9; margin-top:8px;">Veri: {firsat['Kaynak']}</p>
            ...
            st.warning("Piyasada şu an güçlü bir trend (Momentum Rallisi) sinyali bulunamadı. Nakit kraldır.")
